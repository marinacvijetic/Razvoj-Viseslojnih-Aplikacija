...
public class PreduzeceController {
	...
	public Collection<Preduzece> getAllPreduzeces(){
	...
	public ResponseEntity<Preduzece> createPreduzece(@RequestBody Preduzece preduzece){
		if(!repositoryPreduzece.existsById(preduzece.getIdPreduzece())) {
			Preduzece temPreduzece = repositoryPreduzece.save(preduzece);
			return new ResponseEntity<Preduzece>(temPreduzece, HttpStatus.CREATED);
		}else {
	...
	public ResponseEntity<Preduzece> deletePreduzece(@PathVariable int id){
		if(repositoryPreduzece.existsById(id)) {
			if(id == -100) {
				...
				jdbcTemplate.execute("insert into preduzece(\"id_preduzece\", \"naziv\", \"pib\", \"sediste\", \"opis\")" + "values(-100, 'Test naziv', 123456, 'Test sediste', 'Test opis')");
				return new ResponseEntity<Preduzece>(HttpStatus.OK);
			}else {
				repositoryPreduzece.deleteById(id);
				return new ResponseEntity<Preduzece>(HttpStatus.OK);
