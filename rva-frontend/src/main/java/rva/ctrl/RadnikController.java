...
public class RadnikController {
	...
	@GetMapping("/radnik")
	Collection<Radnik> getAllRadnik(){
	...
	public ResponseEntity<Radnik> createRadnik(@RequestBody Radnik radnik){
		if(repositoryRadnik.existsById(radnik.getId())) {
			return new ResponseEntity<Radnik>(HttpStatus.CONFLICT);
		}else {
			Radnik tempRadnik = repositoryRadnik.save(radnik);
			return new ResponseEntity<Radnik>(tempRadnik, HttpStatus.CREATED);
		}
	}
	...
	public ResponseEntity<Radnik> deleteRadnik(@PathVariable int id){
		if(repositoryRadnik.existsById(id)){
			if(id == -100) {
				...
				jdbcTemplate.execute("INSERT INTO radnik(\"id\", \"ime\", \"prezime\", \"broj_lk\", \"obrazovanje\", \"sektor\")" + "VALUES(-100, 'Ime', 'prezime', 123456, 2, 2)");
				return new ResponseEntity<Radnik>(HttpStatus.OK);
			}else {
				repositoryRadnik.deleteById(id);
				return new ResponseEntity<Radnik>(HttpStatus.OK);
